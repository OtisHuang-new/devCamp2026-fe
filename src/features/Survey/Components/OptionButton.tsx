...
function OptionButton({ text, iconPath, isSelected, layout, onClick }: OptionButtonProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-[20px] px-[14px] ${widthClass} h-[55px] rounded box-border cursor-pointer transition-all duration-200
                ${
                  isSelected
    ...
    >
      ...
      <span className="text-[20px] font-bold">{text}</span>
    </div>
  );
}
